...
export default function PublicPricing(){
  ...
  return <section className="pricing section" id="pricing"><div className="section-title"><h2>اشتراک مناسب خود را انتخاب کنید</h2><p>تعرفه‌های ماهانه، سه‌ماهه و سالانه؛ بدون هزینه پنهان</p></div><div className="pricing-wrap container"><div className="pricing-art"><GraduationCap/><h3>یک سرمایه‌گذاری کوچک<br/>برای یک آینده بزرگ</h3><p>همین امروز شروع کنید</p></div>{plans.map((plan,index)=><article className={'price-card '+(index===1&&plan.active!==false?'featured ':'')+(plan.active===false?'subscription-unavailable':'')} key={plan.id}>{index===1&&plan.active!==false&&<span className="discount">انتخاب پیشنهادی</span>}<h3>{plan.name}</h3><div className="price"><b>{plan.price > 0 ? plan.price.toLocaleString('fa-IR') : 'تعرفه اعلام نشده'}</b>{plan.price > 0 && <span>تومان / {periodLabel[plan.period]||plan.period}</span>}</div><ul>{plan.features.map(feature=><li key={feature}><Check/>{feature}</li>)}</ul>{plan.active===false ? <span className="subscription-unavailable-note">این طرح فعلاً در دسترس نیست</span> : <Link className={'btn '+(index===1?'btn-primary':'btn-soft')} to="/register">انتخاب این اشتراک</Link>}</article>)}</div></section>
}
