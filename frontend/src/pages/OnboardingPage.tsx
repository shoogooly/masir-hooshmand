...
function StudentStep({data,refresh,onPartChange}:{data:StatusData;refresh:()=>void;onPartChange:(part:number)=>void}){
 const initial={...data.profile,full_name:data.user.full_name,profile_photo:data.user.profile_photo}
...
function StudentSelection({data,refresh}:{data:StatusData;refresh:()=>void}){
 ...
 const options=useQuery({queryKey:['registration-options',level],queryFn:()=>api<RegistrationOptions>(`/registrations/options?education_level=${level}`)})
 ...
 <div className="choice-cards">{(data.paid&&data.paid_plan?[data.paid_plan]:options.data?.plans)?.map(plan=><label key={plan.id} className={plan.active === false ? 'subscription-unavailable' : ''}><input type="radio" name="plan_id" value={plan.id} checked={planId===plan.id} onChange={()=>setPlanId(plan.id)} disabled={(plan.active === false && !data.paid) || (data.paid && plan.id!==data.selection?.plan_id)} required/><b>{plan.name}</b><span>{plan.price > 0 ? (data.user.referred_by_advisor_id?plan.referral_price||plan.price:plan.price).toLocaleString('fa-IR') + ' تومان' : 'تعرفه اعلام نشده'}</span>{plan.active === false && !data.paid && <em className='subscription-unavailable-note'>این طرح فعلاً در دسترس نیست</em>}<small>{plan.features.join(' · ')}</small></label>)}</div>
 <div className="advisor-selection-field"><b>انتخاب مشاور:</b>{referred?<p>{data.referred_advisor?.full_name||'مشاور معرفی‌کننده'}؛ تعرفه معرفی برای شما اعمال می‌شود.</p>:<><div className="advisor-choice"><label><input type="radio" checked={mode==='admin'} onChange={()=>setMode('admin')}/> انتخاب توسط مدیریت</label><label><input type="radio" checked={mode==='self'} onChange={()=>setMode('self')}/> انتخاب توسط من</label></div>{mode==='self'&&<select aria-label="انتخاب مشاور" value={advisorId} onChange={e=>setAdvisorId(e.target.value)} required><option value="">انتخاب مشاور…</option>{options.data?.advisors.filter(a=>(a.work_levels||[a.education_level]).includes(level as 'lower_secondary'|'upper_secondary')&&!rejected.includes(a.id)).map(a=><option key={a.id} value={a.id} disabled={a.is_full}>{a.full_name} — {a.is_full?'تکمیل ظرفیت':a.remaining_capacity+' ظرفیت باقی‌مانده'}</option>)}</select>}</>}</div>
 {save.error&&<ErrorText error={save.error}/>}<button className="btn btn-primary btn-lg" disabled={save.isPending||!planId||(!data.paid&&options.data?.plans.find(plan=>plan.id===planId)?.active===false)}>ارسال درخواست برای تأیید مشاور</button></form>
}
...
function Payment({payment,refresh}:{payment:NonNullable<StatusData['payment']>;refresh:()=>void}){
 const pay=useMutation({mutationFn:async()=>{const result=await api<{redirect_url?:string;signature?:string}>('/payments/start',{method:'POST',body:JSON.stringify({order_id:payment.order_id})});if(result.redirect_url){window.location.assign(result.redirect_url);return}return api('/payments/callback',{method:'POST',body:JSON.stringify({order_id:payment.order_id,success:true,signature:result.signature})})},onSuccess:refresh})
 return <section className="registration-form onboarding-card payment-step"><CreditCard/><h2>پرداخت طرح انتخابی</h2><p>مبلغ: <b>{payment.amount.toLocaleString('fa-IR')} تومان</b></p>{pay.error&&<ErrorText error={pay.error}/>}<button className="btn btn-primary btn-lg" disabled={pay.isPending} onClick={()=>pay.mutate()}>ورود به درگاه پرداخت</button></section>
}
