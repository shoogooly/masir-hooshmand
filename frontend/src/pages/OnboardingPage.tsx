...
type StatusData = { user: User; step: string; profile?: Record<string, unknown>; payment?: { order_id: string; amount: number; signature: string } }

export default function OnboardingPage({ user }: { user: User }) {
  const queryClient = useQueryClient()
  const status = useQuery({ queryKey: ['onboarding'], queryFn: () => api<StatusData>('/onboarding/status'), refetchInterval: ['lead_review', 'manager_review', 'advisor_confirmation', 'advisor_assignment'].includes(user.onboarding_step || '') ? 15000 : false })
...
function Progress({ role, step }: { role: string; step: string }) {
  const student = [['account', 'ساخت حساب'], ['profile', 'اطلاعات فردی'], ['profile_correction', 'اصلاح اطلاعات'], ['terms', 'پذیرش شرایط'], ['selection', 'طرح و مشاور'], ['payment', 'پرداخت'], ['advisor_confirmation', 'تأیید مشاور'], ['advisor_assignment', 'تخصیص مدیر'], ['completed', 'تکمیل ثبت‌نام']]
...
function StudentStep({ data, refresh }: { data: StatusData; refresh: () => void }) {
  ...
  if (data.step === 'selection') return <StudentSelection refresh={refresh} educationLevel={String(data.profile?.education_level || 'upper_secondary')} referred={Boolean(data.user.referred_by_advisor_id)} />
...
function StudentSelection({ refresh, educationLevel, referred }: { refresh: () => void; educationLevel: string; referred:boolean }) {
  ...
  const [mode, setMode] = useState<'self' | 'admin'>('admin')
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); save.mutate({ plan_id: form.get('plan_id'), advisor_selection_mode: mode, advisor_id: mode === 'self' ? form.get('advisor_id') : null }) }
  if (options.isLoading) return <div className="page-state">در حال دریافت طرح‌ها...</div>
  return <form className="registration-form onboarding-card" onSubmit={submit}><Title title="انتخاب طرح و مشاور" text={`${referred?'شما توسط مشاور به سایت معرفی شده‌اید و تعرفه ویژه برایتان اعمال می‌شود. ':''}می‌توانید یک مشاور فعال و تأییدشده در مقطع خود را انتخاب کنید یا انتخاب را به مدیر سایت بسپارید. انتخاب مستقیم شما باید پس از پرداخت توسط مشاور تأیید شود؛ تخصیص مدیر فوری است. مشاور تکمیل‌ظرفیت قابل انتخاب نیست.`} /><div className="choice-cards">{options.data?.plans.map(plan => <label key={plan.id}><input type="radio" name="plan_id" value={plan.id} required /><b>{plan.name}</b><span>{(referred?plan.referral_price||plan.price:plan.price).toLocaleString('fa-IR')} تومان</span><small>{plan.features.join(' · ')}</small></label>)}</div><div className="advisor-choice"><label><input type="radio" checked={mode === 'admin'} onChange={() => setMode('admin')} /> انتخاب توسط مدیریت</label><label><input type="radio" checked={mode === 'self'} onChange={() => setMode('self')} /> انتخاب توسط من (نیازمند تأیید مشاور)</label></div>{mode === 'self' && <select name="advisor_id" required defaultValue=""><option value="">انتخاب مشاور...</option>{options.data?.advisors.filter(advisor => advisor.education_level === educationLevel).map(advisor => <option key={advisor.id} value={advisor.id} disabled={advisor.is_full}>{advisor.full_name} — {advisor.is_full ? 'تکمیل ظرفیت' : `${advisor.remaining_capacity} ظرفیت باقی‌مانده`}</option>)}</select>}{save.error && <ErrorText error={save.error} />}<button className="btn btn-primary btn-lg" disabled={save.isPending}>ادامه به پرداخت</button></form>
}
