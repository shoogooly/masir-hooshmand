...
export default function StudentSubscriptionOfferPage(){
  ...
  const pay=useMutation({mutationFn:async(value:Exclude<Offer,null>)=>{const result=await api<{redirect_url?:string;signature?:string}>('/payments/start',{method:'POST',body:JSON.stringify({order_id:value.order_id})});if(result.redirect_url){window.location.assign(result.redirect_url);return}return api('/payments/callback',{method:'POST',body:JSON.stringify({order_id:value.order_id,success:true,signature:result.signature})})},onSuccess:()=>{qc.invalidateQueries({queryKey:['me']});qc.invalidateQueries({queryKey:['pending-subscription-offer']});qc.invalidateQueries({queryKey:['notifications']})}})
