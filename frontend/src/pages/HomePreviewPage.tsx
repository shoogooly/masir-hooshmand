  ...
  return <div className="mh" dir="rtl" ref={root} id="preview-top">
    ...
    <main id="main-content">
        ...
        <div className="mh-scene" ref={scene}>
          ...
          <div className="mh-stage">
            ...
            <div className="mh-planner">
              ...
              <div className="mh-plan-bottom"><span><i /> قدم اول انجام شد</span></div><div className="mh-progress"><i /></div>
            </div>
      ...
      <section className="mh-section mh-wrap" id="preview-pricing" ref={pricing}><div className="mh-center mh-reveal"><span className="mh-overline">همراهی، به انتخاب تو</span><h2>طرح مناسب مسیرت را پیدا کن.</h2><p>مدت همراهی، قیمت و امکانات هر اشتراک را بررسی کن.</p></div><div className="mh-prices">
        {plans.isPending ? <div className="mh-price-state" role="status"><span className="mh-loading-dot" /> در حال دریافت تعرفه‌های به‌روز…</div> : plans.isError ? <div className="mh-price-state" role="status"><CreditCard /><h3>تعرفه‌ها فعلاً در دسترس نیستند.</h3><p>برای دریافت قیمت‌های به‌روز دوباره تلاش کن.</p><button className="mh-btn mh-btn-outline" onClick={() => void plans.refetch()} disabled={plans.isFetching}>{plans.isFetching ? 'در حال دریافت…' : 'دریافت دوباره تعرفه‌ها'}</button></div> : visiblePlans.length ? visiblePlans.map((plan) => <article className={'mh-price ' + (plan.period === 'yearly' && plan.active !== false ? 'mh-price-featured ' : '') + (plan.active === false ? 'mh-price-unavailable' : '')} key={plan.id}>{plan.period === 'yearly' && plan.active !== false && <span className="mh-price-popular">انتخاب محبوب</span>}<span className="mh-overline">{periods[plan.period] || plan.period}</span><h3>{plan.name}</h3><div className="mh-price-amount"><strong>{plan.price > 0 ? fa.format(plan.price) : 'تعرفه اعلام نشده'}</strong>{plan.price > 0 && <span>تومان</span>}</div><ul>{plan.features.map(feature => <li key={feature}><Check size={17} />{feature}</li>)}</ul>{plan.active === false ? <span className="mh-price-disabled">این طرح فعلاً در دسترس نیست</span> : <Link className={'mh-btn ' + (plan.period === 'yearly' ? '' : 'mh-btn-outline')} to="/register">انتخاب این طرح <ArrowLeft size={17} /></Link>}</article>) : <div className="mh-price-state"><h3>هنوز طرحی تعریف نشده است.</h3><p>پس از فعال‌شدن اشتراک‌ها، مشخصات و تعرفه‌ها اینجا نمایش داده می‌شود.</p></div>}
      </div></section>
