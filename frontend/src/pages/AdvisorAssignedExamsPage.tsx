...
import OnlineExamSheet,{AnswerKeyBuilder,validateAnswerKey} from './OnlineExamSheet'
import type {AnswerSectionDraft} from './onlineExamTypes'
...
export default function AdvisorAssignedExamsPage({studentId}:{studentId:string}){
  ...
  const [online,setOnline]=useState(true),[negative,setNegative]=useState(false)
  const [sections,setSections]=useState<AnswerSectionDraft[]>([{id:'first',title:'',correct_answers:[null]}])
  const exams=useQuery({queryKey:['assigned-exams',studentId],queryFn:()=>api<AssignedExam[]>(`/assigned-exams?student_id=${studentId}`),refetchInterval:10000})
  const create=useMutation({
    mutationFn:async(form:HTMLFormElement)=>{
      ...
      const online_sheet=online?{sections:sections.map(({title,correct_answers})=>({title:title.trim(),correct_answers})),negative_marking:negative}:null
      if(online_sheet)validateAnswerKey(online_sheet)
      return api<AssignedExam>(`/advisors/students/${studentId}/assigned-exams`,{method:'POST',body:JSON.stringify({title:data.get('title'),duration_minutes:data.get('duration')?Number(data.get('duration')):null,instructions:data.get('instructions'),question_file:await pdfPayload(file),online_sheet})})
    },
    onMutate:()=>setCreateSent(false),
    onSuccess:(item,form)=>{form.reset();setSections([{id:crypto.randomUUID(),title:'',correct_answers:[null]}]);setNegative(false);setCreateSent(true);setSelected(item.id);qc.invalidateQueries({queryKey:['assigned-exams',studentId]})},
  })
  ...
  return <div className="advisor-exam-page">
    <form className="panel exam-create-form" onSubmit={event=>{event.preventDefault();create.mutate(event.currentTarget)}}>
      ...
      <label className="full online-check"><input type="checkbox" checked={online} onChange={e=>setOnline(e.target.checked)}/> پاسخنامه آنلاین چهارگزینه‌ای</label>
      {online&&<AnswerKeyBuilder sections={sections} setSections={setSections} negative={negative} setNegative={setNegative}/>}
      <button className="btn btn-primary" disabled={create.isPending}><Send/> ارسال آزمون برای دانش‌آموز</button><ExamError error={create.error}/>
    ...
    {exams.isLoading?<div className="page-state">در حال دریافت آزمون‌ها...</div>:<div className="advisor-exam-history"><h2>سوابق آزمون‌های دانش‌آموز</h2>{exams.data?.map(exam=><article className={`panel exam-history-item ${selected===exam.id?'open':''}`} key={exam.id}>
      ...
      {selected===exam.id&&<div className="exam-history-details">
        {exam.has_online_sheet&&<OnlineExamSheet examId={exam.id} advisor/>}
        ...
        {exam.answer_uploaded_at&&<>{!exam.has_online_sheet&&<div className="exam-submission"><FileCheck2/><div><b>{exam.answer_filename}</b><p>{exam.student_notes||'دانش‌آموز توضیحی ثبت نکرده است.'}</p></div><button className="btn btn-outline" onClick={()=>download.mutate(exam.id)}><Download/> دانلود پاسخنامه</button></div>}
