...
export default function AdminSubscriptionSettingsPage(){
  ...
  return <div className="content-page"><div className="section-head"><div><h1>تعرفه‌ها و مدیریت اشتراک</h1><p>مبلغ سه طرح اصلی و تعرفه ویژه معرفی‌شده توسط مشاور را تعیین کنید.</p></div><Link className="btn btn-outline" to="/app/admin/ledger">مشاهده واریزی‌ها و مانده اشتراک‌ها</Link></div>
    <div className="subscription-admin-grid">{finance.data.plans.map(plan=><form className="panel subscription-editor" key={plan.id} onSubmit={event=>savePlan(event,plan.id)}><CreditCard/><h2>{plan.name}</h2><label>مبلغ عادی (تومان)<input name="price" type="number" min="0" required defaultValue={plan.price}/></label><label>مبلغ معرفی توسط مشاور<input name="referral_price" type="number" min="0" required defaultValue={plan.referral_price||plan.price}/></label><label className="switch-line"><input name="active" type="checkbox" defaultChecked={plan.active}/> قابل خرید</label><button className="btn btn-primary" disabled={update.isPending}><Save/> ذخیره تعرفه</button></form>)}</div>
    Cannot overwrite variable Error because it is read-only or constant.
    {update.error&&<div className="form-error">{update.error instanceof Error?update.error.message:'ذخیره تعرفه ناموفق بود'}</div>}
