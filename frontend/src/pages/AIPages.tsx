...
type Usage={weekly_auto_enabled?:boolean;limit:number;used:number;remaining:number;reset_at:string;locked:boolean;enabled:boolean;override:number|null;default_limit:number}
...
function SettingsForm({config}:{config:Config}){
 ...
 return <form ref={form} className="ai-settings-form" onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);save.mutate({provider,enabled:f.get('enabled')==='on',model:f.get('model'),weekly_limit:Number(f.get('limit')),api_key:f.get('token')||null,clear_token:f.get('clear')==='on'})}}>
  ...
  <label className="ai-check"><input type="checkbox" name="enabled" defaultChecked={config.enabled}/> فعال‌سازی سرویس هوش مصنوعی</label>
...
function StudentLimit({student}:{student:Student}){
 const toggle=useMutation({mutationFn:()=>api('/ai/students/'+student.id+'/weekly-review',request({enabled:!student.usage.weekly_auto_enabled},'PUT')),onSuccess:()=>void qc.invalidateQueries({queryKey:['ai-students']})})
...
export function AdminAIPage(){
 ...
 return <div className="content-page ai-page"><Intro title="مدیریت هوش مصنوعی">اتصال گپ‌جی‌پی‌تی و میسترال و کنترل سهمیهٔ گفت‌وگوی درسی دانش‌آموزان.</Intro><ErrorMessage error={config.error}/>{config.data&&<SettingsForm config={config.data}/>}<section className="ai-limit-panel"><h2>سهمیه و تحلیل هفتگی دانش‌آموزان</h2><p>تحلیل خودکار پیش‌فرض خاموش است؛ فقط برای دانش‌آموزان مورد نظر آن را روشن کنید. بررسی دستی مشاور مستقل از این کلید است.</p><p>برای استفاده از پیش‌فرض عمومی، فیلد سهمیه را خالی بگذارید.</p><label className="ai-search"><Search size={18}/><input aria-label="جست‌وجوی سهمیه دانش‌آموز" placeholder="جست‌وجوی نام…" value={search} onChange={e=>setSearch(e.target.value)}/></label><ErrorMessage error={students.error}/>{students.data?.filter(s=>s.full_name.includes(search)).map(s=><StudentLimit key={s.id+':'+s.usage.override+':'+s.usage.default_limit} student={s}/>)}</section></div>
}
