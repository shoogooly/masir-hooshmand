...
import OnlineExamSheet from './OnlineExamSheet'
...
export default function StudentAssignedExamsPage(){
  ...
  const exams=useQuery({queryKey:['assigned-exams'],queryFn:()=>api<AssignedExam[]>('/assigned-exams'),refetchInterval:10000})
  const download=useMutation({
    mutationFn:({id,kind}:{id:string;kind:'question'|'lesson'})=>api<FileResult>(`/assigned-exams/${id}/${kind}-file`),
    onSuccess:file=>{saveFile(file);qc.invalidateQueries({queryKey:['assigned-exams']});qc.invalidateQueries({queryKey:['online-sheet']})},
  })
  ...
  return <section className="assigned-exam-section">
    <div className="section-head compact"><div><h1>آزمون‌های ارسالی مشاور</h1><p>سؤالات را دانلود کنید و پاسخنامه آنلاین یا PDF تعیین‌شده توسط مشاور را تکمیل کنید.</p></div></div>
    ...
    <div className="advisor-exam-history student-exam-history">{exams.data?.map(exam=><article className={`panel exam-history-item ${selected===exam.id?'open':''}`} key={exam.id}>
      ...
      {selected===exam.id&&<div className="exam-history-details">
        ...
        <ExamError error={download.error}/>
        {exam.has_online_sheet?<OnlineExamSheet examId={exam.id}/>:!exam.answer_uploaded_at?<form className="exam-answer-form" onSubmit={event=>{event.preventDefault();answer.mutate({id:exam.id,form:event.currentTarget})}}><label>فایل پاسخنامه PDF<input name="answer" type="file" accept="application/pdf,.pdf" required/></label><label>توضیحات شما درباره آزمون<textarea name="notes" maxLength={5000} placeholder="بخش‌های دشوار یا توضیحی برای مشاور..."/></label><button className="btn btn-primary" disabled={answer.isPending}><Upload/> بارگذاری پاسخنامه</button><ExamError error={answer.error}/></form>:
