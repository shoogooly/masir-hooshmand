...
export function AdminUsersPage(){
  ...
  return <div className="content-page"><Head title="مدیریت کاربران" subtitle="مشاهده و کنترل وضعیت تمام حساب‌های سامانه"/><div className="admin-toolbar"><div className="list-search"><Search/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="نام یا شماره موبایل..."/></div><select value={role} onChange={e=>setRole(e.target.value)}><option value="all">همه نقش‌ها</option><option value="student">دانش‌آموزان</option><option value="advisor">مشاوران</option><option value="expert">کارشناسان</option><option value="secretary">منشی‌ها</option><option value="super_admin">مدیران</option></select></div><div className="admin-table"><div className="admin-table-head"><span>کاربر</span><span>نقش</span><span>وضعیت</span><span>شماره</span><span>اقدام</span></div>{rows.map(item=><div className="admin-table-row" key={item.id}><span><i className="avatar">{item.full_name[0]}</i><b>{item.full_name}</b></span><span>{item.role}</span><Badge value={item.status}/><span dir="ltr">{item.phone}</span><select value={item.status} disabled={update.isPending} onChange={e=>update.mutate({id:item.id,status:e.target.value})}><option value="active">فعال</option><option value="suspended">غیرفعال</option><option value="pending_assignment">در انتظار تخصیص</option><option value="pending_approval">در انتظار تأیید</option></select></div>)}</div>{update.error&&<ErrorBox error={update.error}/>}</div>
}
...
export function AdminAccessPage(){
  const access=[['مدیر سایت','دسترسی کامل به کاربران، برنامه‌ها، گفت‌وگوها، تخصیص و انتقال، تأیید و امور مالی'],['کارشناس','دسترسی به مشاوران تعیین‌شده و دانش‌آموزان همان مشاوران؛ سطح هر بخش از پنل کارکنان تنظیم می‌شود'],['منشی','سطح مشاهده یا ویرایش هر بخش از پنل کارکنان تنظیم می‌شود'],['مدیر عملیات','کاربران، پرونده‌ها، تخصیص و گزارش‌های عملیاتی'],['مدیر مالی','طرح‌ها، سفارش‌ها و پرداخت‌ها']]
