...
export function SettingsPage({user:_user}:{user?:User}={}){
  const qc=useQueryClient(),[schedule,setSchedule]=useState<Record<string,string[]>>({}),[extras,setExtras]=useState<Record<string,string>>({})
  const {data,isLoading,error}=useQuery({queryKey:['profile'],queryFn:()=>api<Profile>('/profile')})
  useEffect(()=>{if(data?.role==='student'){setSchedule(data.school_schedule||{});setExtras(data.extra_classes||{})}},[data])
  const save=useMutation({mutationFn:(body:Record<string,unknown>)=>api('/profile',{method:'PATCH',body:JSON.stringify(body)}),onSuccess:()=>{qc.invalidateQueries({queryKey:['profile']});qc.invalidateQueries({queryKey:['me']})}})
  if(isLoading)return <State/>;if(error||!data)return <ErrorBox error={error}/>
  const submit=(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget),body:Record<string,unknown>={full_name:form.get('full_name')}
    ...
    if(data.role==='advisor')Object.assign(body,{education_degree:form.get('education_degree'),education_field:form.get('education_field'),experience_years:Number(form.get('experience_years')),support_capacity:Number(form.get('support_capacity')),academic_year:form.get('academic_year'),bio:form.get('bio')})
    save.mutate(body)}
