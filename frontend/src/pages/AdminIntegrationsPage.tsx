...
export default function AdminIntegrationsPage(){
 ...
 useEffect(()=>{if(query.data)setForm(x=>({...x,sms_enabled:query.data!.sms_enabled,sms_template_id:query.data!.sms_template,sms_parameter_name:query.data!.sms_parameter||'Code',zarinpal_enabled:query.data!.zarinpal_enabled,zarinpal_sandbox:query.data!.zarinpal_sandbox,public_url:query.data!.public_url}))},[query.data])
 ...
 return <div className="content-page"><div className="section-head"><div><h1>پیامک و درگاه پرداخت</h1><p>اتصال SMS.ir برای کدهای یک‌بارمصرف و زرین‌پال برای پرداخت واقعی سایت.</p></div></div>
  <div className="integration-grid"><section className="panel integration-card"><header><MessageSquareText/><div><h2>SMS.ir</h2><small>{query.data?.sms_key_configured?'کلید ذخیره شده':'کلید تنظیم نشده'}</small></div></header>
   ...
   <label>شناسه قالب Verify<input value={form.sms_template_id} onChange={e=>setForm({...form,sms_template_id:e.target.value.replace(/\D/g,'')})} inputMode="numeric" placeholder="مثلاً 123456"/></label>
   <label>نام پارامتر کد<input value={form.sms_parameter_name} onChange={e=>setForm({...form,sms_parameter_name:e.target.value})} dir="ltr" placeholder="Code"/></label>
   <small>برای ارسال شبانه‌روزی، ابتدا در بخش «ارسال سریع» پنل SMS.ir یک قالب Verify بسازید. شناسه قالب و نام پارامتر باید دقیقاً مطابق همان قالب باشند؛ معمولاً نام پارامتر <b>Code</b> است.</small>
  </section>
